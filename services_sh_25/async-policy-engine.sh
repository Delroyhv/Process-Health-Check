...
POLICY_SERVICE_PORT=${!BOUND_PORT_DEF_POLICY_SERVICE_PORT}
...
# Redirect error logs
outlog="$SERVICE_LOG_DIR/async-policy-service.stdout"
errlog="$SERVICE_LOG_DIR/async-policy-service.stderr"

redirect_to_log_int "$outlog" "$errlog"

/opt/aspen/scripts/start-vertx.sh "$LOG4J2_CONF_DIR" "$VERTX_OPTS" "$MONITORING_PORT" "$SUPPORT_PORT" "$DEBUG_PORT" "$JMX_PORT" "$POLICY_SERVICE_PORT"
