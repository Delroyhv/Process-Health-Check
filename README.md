...
```bash
sudo ./gsc_healthcheck.sh -c ACME -s 17762026 -f /path/to/supportLogs_BUNDLE.tar.xz
```
...
3.  **Run Prometheus Setup (`gsc_prometheus.sh`)**:
    ...
    *   **Example Call:** `sudo gsc_prometheus.sh -c ACME -s 17762026 -f psnap_FILE.tar.xz -b .`
...
4.  **Execute Health Check Suite (`runchk.sh`)**:
    ...
    *   **Example Call:** `runchk.sh -f healthcheck.conf [--no-metrics] [--report ACME_report.md]`
    *   **Result:** The full suite of health checks is performed and a summary is printed to the console. Partition growth rates (including a 6-month average monthly growth trend) are calculated and logged to `partition_growth_chart.log`. When `--report` is given, a Markdown report is generated by `gsc_healthcheck_report.sh`. If Prometheus is unreachable, `chk_metrics.sh` is skipped with a `[WARN]` message.
